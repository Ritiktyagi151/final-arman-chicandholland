...
// Load JSZip BEFORE dynamic import
...
const downloadExcel = () => {
  const data = fullExpensesData.map((exp) => ({
    ...
    Recipient: exp.payer,
    "Expense Type":
      exp.expenseType + (exp.otherType ? ` - ${exp.otherType}` : ""),
    Amount: exp.amount,
  ...
  for (let R = 1; R <= range.e.r; R++) {
    for (let C = range.s.c; C <= range.e.c; C++) {
      ...
      cell.s = {
        fill: {
          fgColor: {
            rgb: R % 2 === 0 ? "F9FAFB" : "FFFFFF", // zebra rows
          },
        },
        alignment: {
          vertical: "center",
...
const downloadPPT = async () => {
  ...
  slide.addTable([tableHeaders, ...tableRows], {
    ...
    autoPage: false, // 👈 IMPORTANT: Prevent multiple slides
    rowH: 0.18, // 👈 Shrink row height
    colW: [1.1, 1, 1.4, 2.4, 0.7, 0.7, 0.8],
  });

  // SUMMARY (on same slide)
  const summaryText = expenses.totalAmount
    .map((t) => `${t.currency.toUpperCase()}: ${t.totalAmount}`)
    .join("     ");

  slide.addText(`Summary:  ${summaryText}`, {
    x: 0.3,
    y: 6.5,
    fontSize: 14,
    bold: true,
  ...
  return (
    <div className="justift-end w-fll flex space-x-2">
      ...
      <Button onClick={downloadPPT} variant="outline">
        Download PPT
      </Button>
    </div>
  );
